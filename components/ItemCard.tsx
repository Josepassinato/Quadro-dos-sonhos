...
const ItemCard: React.FC<ItemCardProps> = ({ item, onUpdate, onRemove }) => {
    ...
    return (
        <div 
            className={`group relative aspect-w-1 aspect-h-1 rounded-lg overflow-hidden shadow-lg transition-all duration-300 ${isSaved ? 'ring-2 ring-offset-2 ring-green-500 dark:ring-offset-slate-800' : ''}`}
        ...
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 text-white z-10">
                {isEditing ? (
                    <div className="flex items-center gap-1">
                        ...
                        />
                        <button onClick={handleSave} className="p-1 rounded-full bg-white/20 hover:bg-white/40">
            ...
            <div className={`absolute top-2 right-2 flex gap-2 transition-opacity duration-300 z-10 ${isHovered && !isEditing ? 'opacity-100' : 'opacity-0'}`}>
                <button onClick={() => setIsEditing(true)} className="p-2 rounded-full bg-black/50 hover:bg-black/70 text-white">
